...
describe('inputValues utilities', () => {
  describe('inputsToValues', () => {
    ...
    it('converts single input to object', () => {
      const inputs = [{ id: 'test', value: '5', label: 'Test', image: '', placeholder: '' }]
    ...
    it('converts inputs with empty value to empty string', () => {
      const inputs = [{ id: 'test', value: '', label: 'Test', image: '', placeholder: '' }]
      const result = inputsToValues(inputs)
      expect(result).toEqual({ test: '' })
    })

    it('converts inputs with undefined value to empty string', () => {
      const inputs = [{ id: 'test', value: undefined as unknown as string, label: 'Test', image: '', placeholder: '' }]
      const result = inputsToValues(inputs)
      expect(result).toEqual({ test: '' })
    })
  })
  ...
  describe('calculateTotal', () => {
    ...
    it('calculates total from single value', () => {
      const inputs = [{ id: 'coins', value: '', label: 'Coins', image: '', placeholder: '' }]
  ...
  describe('data integrity', () => {
    ...
    it('all input IDs are unique', () => {
      const allIds = [...initialInputs, ...agoraInputs, ...pantheonInputs].map(i => i.id)
